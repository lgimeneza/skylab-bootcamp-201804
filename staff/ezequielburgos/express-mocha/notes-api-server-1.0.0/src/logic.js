...
const logic = {
    ...
    findNotes(text) {
        ...
        // const notes = this._notes.filter(note => note.text.split(text).length > 1)
        // const notes = this._notes.filter(note => note.text.search(text) !== -1)
        const notes = this._notes.filter(note => note.text.includes(text) /*=== true*/)
