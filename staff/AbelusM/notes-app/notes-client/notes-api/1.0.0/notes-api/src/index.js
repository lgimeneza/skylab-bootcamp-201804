    ...
    updateUser(id, name, surname, email, password, newEmail, newPassword) {
        return Promise.resolve()
            .then(() => {
                ...
                return axios.patch(`${this.url}/users/${id}`, { id, name, surname, email, password, newEmail, newPassword }, { headers: { authorization: `Bearer ${this.token}` } })
            })
            .then(({ status, data }) => {
                if (status !== 200 || data.status !== 'OK') throw Error(`unexpected response status ${status} (${data.status})`)
       ...
       listNotes(userId) {
        return Promise.resolve()
            .then(() => {
                ...
                return axios.get(`${this.url}/users/${userId}/notes`, { userId }, { headers: { authorization: `Bearer ${this.token}` }})
                    .then(({ status, data }) => {
                        if (data.status !== 'OK') throw Error(`unexpected response status ${status} (${data.status})`)
                        return data.data
                    }).catch(({ response: { data: { error } } }) => error)
    ...
    removeNote(userId, noteId) {
        return Promise.resolve()
            .then(() => {
                ...
                return axios.delete(`${this.url}/users/${userId}/notes/${noteId}`, { userId }, { headers: { authorization: `Bearer ${this.token}` }})
                    .then(({ status, data }) => {
                        if (data.status !== 'OK') throw Error(`unexpected response status ${status} (${data.status})`)
                        return true
                    }).catch(({ response: { data: { error } } }) => error)
