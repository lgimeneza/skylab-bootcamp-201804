    ...
    updateUser(id, name, surname, email, password, newEmail, newPassword) {
        return Promise.resolve()
            .then(() => {
                ...
                return axios.patch(`${this.url}/users/${id}`, {name, surname, email, password, newEmail, newPassword}) // id= id de usuario de la BD q lo crea automaticamente mongo + los datos que nos envian por formulario por el body
                    //.then(({ data: { data: user } }) => user)
                    // console.log(id)
                    .then(({ status, data }) => {
