...
describe('logic (notes)', () => {
    ...
    describe('add note', () => {
        ...
        it('should add notes with different ids', (done) => {
            ...
            let p1 =logic.addNote(userId, 'my note 1')
            let p2 =logic.addNote(userId, 'my note 2')
            let p3 =logic.addNote(userId, 'my note 3')
            ...
            .then((res)=>{
                cl.findOne({ _id: ObjectId(res[0]) })
                        .then(data => {expect(data.text).toBe('my note 1')})
            
            
                cl.findOne({ _id: ObjectId(res[1]) })
                        .then(data => {expect(data.text).toBe('my note 2')})
            
            
                cl.findOne({ _id: ObjectId(res[2]) })
                        .then(data => {expect(data.text).toBe('my note 3')})
    ...
    describe('list notes', () => {
            ...
            .then((ids)=>{
                ...
                .then((notes)=>{
                    ...
                    notes.forEach((p, index)=>{
                       ...
                       data.push( cl.findOne({_id: ObjectId(notes[index])})
                            .then(res=> {expect(res._id).toBe(res[index])}))
                    
                        data.push(cl.findOne({ _id: ObjectId(notes[index]) })
                            .then(res => {expect(res.text).toBe(`my note ${index+1}`)}))
                    
                        data.push(cl.findOne({ _id: ObjectId(notes[index]) })
                            .then(res => {expect(res.userId).toBe(userId)}))
                    })
                    Promise.all(data)
        ...
        // let p4 =logic.addNote(userId2, 'my note 4')
        // let p5 =logic.addNote(userId2, 'my note 5')
        // let p6 =logic.addNote(userId2, 'my note 6')
        // let p7 =logic.addNote(userId2, 'my note 7')
        ...
        //         cl.findOne({ _id: ObjectId(data[index]) })
        //             .then(res => {expect(res.id).toBe(res[index])})

        //         cl.findOne({ _id: ObjectId(data[index]) })
        //             .then(res => {expect(res.text).toBe(`my note ${index + 4}`)})
                
        //         cl.findOne({ _id: ObjectId(data[index]) })
        //             .then(res => {expect(res.userId).toBe(userId2)})
